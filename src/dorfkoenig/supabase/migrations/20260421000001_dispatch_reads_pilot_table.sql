-- Switch dispatch_auto_drafts() from Vault-secret parsing to reading the
-- bajour_pilot_villages_list table directly. Behaviour contract unchanged:
--   * empty table → pilot_list = '{}' → ARRAY([]) which never matches
--     any village_id (aligns with the UI's "empty array = disable all").
--   * table with rows → pilot_list contains those ids → village_id = ANY(pilot_list)
-- Previous Vault secret can be deleted by hand post-deploy via:
