-- Fix canonical unit dedup regression.
--
-- Embedding-only canonical merges can collapse unrelated short facts into one
-- stale row. Keep exact duplicate handling, but require lexical similarity for
-- semantic merges. Also track merged PDF units separately from newly-created
-- canonical units so the UI can report duplicates honestly.
...
ALTER TABLE newspaper_jobs
  ADD COLUMN IF NOT EXISTS units_merged INTEGER NOT NULL DEFAULT 0;
