-- External published drafts + soft-dedup signal.
--
-- 1. bajour_drafts gains `provider` ('auto' | 'external') and `published_at`.
--    `provider` distinguishes drafts created by the auto pipeline from drafts
--    pasted in by an editor as the actual published version. `published_at` is
--    the durable signal — set when the draft is confirmed sent (today by the
--    external upload path, long-term by an external API webhook).
--
-- 2. unit_occurrences gains `draft_id`. This is the load-bearing piece for the
--    soft-dedup query in bajour-auto-draft: "does this candidate unit appear in
--    a draft this village published in the last N days?". Without it, the
--    lookup falls back to array-contains scans on bajour_drafts.selected_unit_ids.
--
-- 3. upsert_canonical_unit gains `p_draft_id` and writes it into the new
--    unit_occurrences column. Existing call sites are unaffected (default NULL).
--
-- 4. Two narrow indexes support the EXISTS-join lookup at selection time.

BEGIN;

SET search_path = public, extensions;

-- ── bajour_drafts: provider + published_at ────────────────────────────────
...
COMMENT ON COLUMN bajour_drafts.provider IS
  'auto = produced by bajour-auto-draft pipeline; external = pasted in by editor as the actually-published version (temporary bridge until external API marks Dorfkönig drafts as published).';
COMMENT ON COLUMN bajour_drafts.published_at IS
  'Set when the draft is confirmed sent to the audience. Drives soft-dedup of recently-published units in subsequent auto-drafts for the same village.';

CREATE INDEX IF NOT EXISTS idx_bajour_drafts_published_village
  ON bajour_drafts (village_id, published_at DESC)
  WHERE published_at IS NOT NULL;

-- ── unit_occurrences: draft_id link ───────────────────────────────────────

ALTER TABLE unit_occurrences
  ADD COLUMN IF NOT EXISTS draft_id UUID REFERENCES bajour_drafts(id) ON DELETE SET NULL;

COMMENT ON COLUMN unit_occurrences.draft_id IS
  'Set when an occurrence was extracted from an external published draft. Joined against bajour_drafts.published_at for the soft-dedup signal in bajour-auto-draft selection.';

CREATE INDEX IF NOT EXISTS idx_unit_occurrences_draft_id
  ON unit_occurrences (draft_id)
  WHERE draft_id IS NOT NULL;

-- ── upsert_canonical_unit: gain p_draft_id ────────────────────────────────
-- Drop the previous signature first; PG will not let us add a parameter in
-- the middle of a defaulted positional list without ambiguity.

DROP FUNCTION IF EXISTS upsert_canonical_unit(
