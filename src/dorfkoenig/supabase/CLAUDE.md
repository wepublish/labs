...
| `bajour-auto-draft` | Automated daily draft pipeline per village (select → compose → validate → save → verify). Compose routed through `_shared/compose-draft.ts`. Feature flags: `FEATURE_BULLET_SCHEMA`, `FEATURE_QUALITY_GATING`, `FEATURE_EMPTY_PATH_EMAIL`, `FEATURE_METRICS_CAPTURE`. Empty paths (no_units / all_below_quality_threshold / llm_under_produced) email `ADMIN_EMAILS` via `buildDraftFailureEmail`. Pre-flight `loadPreviouslyPublishedDates()` joins `unit_occurrences ↔ bajour_drafts` to find which candidates appeared in a published draft for this village in the last 14 days; matches gain a `PUBLISHED:YYYY-MM-DD` token in the LLM selection prompt for soft dedup. Computes `publicationDate = today + 1` and threads it into both selection and v2 compose prompts so the LLM frames event times from the reader's perspective on the publication date (newsletter ships next morning). Event end-window in the compound date filter is `+7d` (tightened from +14d on 2026-04-26 after Tom's feedback that 3–5-day-out events surfaced too early). | Service role (pg_cron) | pg_cron dispatch |
