...
| `bajour-drafts` | CRUD for Bajour village newsletter drafts (GET list, POST create, PATCH update). POST accepts `provider: 'external'` to capture an actually-published newsletter; the handler then extracts atomic units from the body and routes them through `upsertCanonicalUnit()` with `draft_id` set for soft dedup. It does not seed compose few-shot examples; those come from finished bullets captured through draft feedback. Sets `published_at = now()` on external drafts. | x-user-id header | Frontend API calls / `npm run submit:published` CLI |
| `bajour-select-units` | AI-powered selection of relevant information units for a village | x-user-id header | Frontend API calls |
| `bajour-auto-draft` | Automated daily draft pipeline per village (select → compose → validate → quality-gate → save → verify). Compose routed through `_shared/compose-draft.ts`; post-compose policy lives in `_shared/auto-draft-quality.ts`. Feature flags: `FEATURE_BULLET_SCHEMA`, `FEATURE_QUALITY_GATING`, `FEATURE_EMPTY_PATH_EMAIL`, `FEATURE_METRICS_CAPTURE`. Empty paths (no_units / all_below_quality_threshold / llm_under_produced) email `ADMIN_EMAILS` via `buildDraftFailureEmail`. Generated-but-weak drafts save with `verification_status='withheld'`, persist `quality_warnings`, do not mark units used, do not send WhatsApp, and email `ADMIN_EMAILS` via `buildDraftWithheldEmail`. Selection now dedupes near-identical candidates before LLM selection and applies the near-term `event_date` window only to `unit_type='event'`; facts/entity updates use publication/created freshness so civic news is not expired as an event. Pre-flight `loadPreviouslyPublishedDates()` joins `unit_occurrences ↔ bajour_drafts` to find which candidates appeared in a published draft for this village in the last 14 days; matches gain a `PUBLISHED:YYYY-MM-DD` token in the LLM selection prompt for soft dedup. Computes reader-facing publication date once and threads it into both selection and v2 compose prompts. Event end-window in the compound date filter is `+7d`. | Service role (pg_cron) | pg_cron dispatch |
...
- PK: `id` (UUID), `user_id` (TEXT), `village_id`, `village_name`, `title`, `body`, `selected_unit_ids` (UUID[]), `custom_system_prompt`, `publication_date` (DATE, default CURRENT_DATE), `verification_status` (ausstehend/bestätigt/abgelehnt/withheld), `verification_responses` (JSONB[]), `verification_sent_at`, `verification_resolved_at`, `verification_timeout_at`, `whatsapp_message_ids` (JSONB), `quality_warnings` (JSONB)
