...
| `bajour-drafts` | CRUD for Bajour village newsletter drafts (GET list, POST create, PATCH update). POST accepts `provider: 'external'` to capture an actually-published newsletter; the handler then extracts atomic units from the body, routes through `upsertCanonicalUnit()` with `draft_id` set, and seeds `bajour_feedback_examples` with `kind='positive'` rows so the existing `FEATURE_FEEDBACK_RETRIEVAL` consumer picks them up without code changes. Sets `published_at = now()` on external drafts. | x-user-id header | Frontend API calls / `npm run submit:published` CLI |
