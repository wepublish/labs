...
- PK: `id` (UUID), `user_id` (TEXT), `storage_path` (TEXT), `publication_date` (DATE), `source_url` (TEXT, required by new manual PDF uploads), `label` (TEXT), `source_type` (manual_pdf/manual_text), `status` (processing/completed/failed/review_pending/storing/cancelled), `stage` (parsing_pdf/chunking/extracting/storing, nullable), `chunks_total`, `chunks_processed`, `units_created`, `units_merged`, `dedup_summary` (JSONB), `skipped_items` (TEXT[]), `extracted_units` (JSONB), `error_message`, `created_at`, `completed_at`
...
7b. **newspaper_jobs hosts text and PDF** -- Despite the name, `newspaper_jobs` now stages both manual-PDF and manual-text extractions (`source_type` discriminates). Text flow: `handleTextUpload` extracts via LLM, stages as `extracted_units` JSONB with status=`review_pending`, reuses `PdfReviewPanel` in the UI and `handlePdfFinalize` on the server. New PDF jobs must carry `source_url`; finalized PDF units use it as both `source_url` and `article_url` so draft citation and quality scoring see real provenance instead of `manual://pdf`. `handlePdfFinalize` also writes `dedup_summary` JSONB so the UI can show which selected review units were in-batch duplicates or merged into existing canonical units. Rename deferred.
