...
import { sanitizeReviewUnit, sanitizeReviewUnits } from '../_shared/manual-upload-review.ts';
  ...
  try {
    ...
    if (req.method === 'GET') {
      ...
      if (recentParam !== null) {
        ...
        const { data, error } = await supabase.from('newspaper_jobs').select('id, label, created_at, status, units_created').eq('user_id', userId).order('created_at', {
          ascending: false
        }).limit(limit);
      ...
      const { data, error } = await supabase.from('newspaper_jobs').select('*').eq('id', jobId).eq('user_id', userId).maybeSingle();
...
// ── Rate Limiting ──────────────────────────────────────────────
async function checkRateLimit(supabase, userId, uploadType) {
  const oneHourAgo = new Date(Date.now() - 3600000).toISOString();
  const { count } = await supabase.from('upload_rate_limits').select('*', {
    count: 'exact',
    head: true
  }).eq('user_id', userId).eq('upload_type', uploadType).gte('created_at', oneHourAgo);
  return (count || 0) < RATE_LIMITS[uploadType];
}
async function recordRateLimit(supabase, userId, uploadType) {
...
// ── Text Upload ────────────────────────────────────────────────
async function handleTextUpload(supabase, userId, body) {
  // Rate limit check
  ...
  const sourceTitle = body.source_title || null;
  ...
  const { data: job, error: jobErr } = await supabase.from('newspaper_jobs').insert({
    user_id: userId,
    // The table still requires a non-null storage_path. Manual text jobs do
    // not have an underlying file, so keep this empty rather than null.
    storage_path: '',
...
AUSGABEFORMAT (JSON):
{
  ...
  const staged = units.filter((u)=>u.statement && u.statement.trim().length > 0).flatMap((u)=>{
...
async function handleFileUploadRequest(supabase, userId, body) {
  // Rate limit check
...
async function handleFileConfirm(supabase, userId, body) {
  const contentType = body.content_type;
  ...
  const { data: fileData, error: downloadError } = await supabase.storage.from('uploads').download(storagePath);
  ...
  if (isPdf) {
    ...
    const { data: job, error: jobError } = await supabase.from('newspaper_jobs').insert({
      user_id: userId,
      storage_path: storagePath,
      publication_date: publicationDate,
...
async function handlePdfFinalize(supabase, userId, body) {
  const jobId = body.job_id;
  const selectedUids = body.selected_uids;
  ...
  const { data: claimed, error: claimErr } = await supabase.from('newspaper_jobs').update({
    status: 'storing'
  }).eq('id', jobId).eq('user_id', userId).eq('status', 'review_pending').select('id, extracted_units, storage_path, label, source_type, publication_date').maybeSingle();
  if (claimErr) {
  ...
  if (!claimed) {
    ...
    const { data: current } = await supabase.from('newspaper_jobs').select('status, units_created, error_message').eq('id', jobId).eq('user_id', userId).maybeSingle();
    if (!current) return errorResponse('Job nicht gefunden', 404);
    if (current.status === 'completed') {
      return jsonResponse({
        data: {
          units_created: current.units_created,
          already_finalized: true
        }
      });
    }
    return errorResponse(`Job ist nicht mehr zur Freigabe bereit (Status: ${current.status})`, 409, 'INVALID_STATE');
  }
  const staged = Array.isArray(claimed.extracted_units) ? claimed.extracted_units : [];
  const byUid = new Map(staged.flatMap((u)=>{
  ...
  const chosenRaw = selectedUids.map((uid)=>byUid.get(uid)).filter((u)=>u !== undefined);
...
async function handlePdfCancel(supabase, userId, body) {
  const jobId = body.job_id;
  if (!jobId) return errorResponse('job_id erforderlich', 400, 'VALIDATION_ERROR');
  const { error } = await supabase.from('newspaper_jobs').update({
    status: 'cancelled',
    extracted_units: null,
    completed_at: new Date().toISOString()
