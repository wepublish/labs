...
| OQ1 | Which newsroom is the pilot for `newsroom:pilot`? | Tom's call; dataset stays empty meanwhile (profiles in `internal` already serve devs) — when chosen, copy its profile + ingest its asserted knowledge via kb-ingest |
